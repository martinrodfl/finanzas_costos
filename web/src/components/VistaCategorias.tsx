...
export default function VistaCategorias({
	...
	const resolverCategoria = (mov: Movimiento): Categoria => {
	...
	const cambiarCategoria = async (id: number, categoria: string | null) => {
		setGuardando(id);
		try {
			await api.patch(`/movimientos/${id}/categoria`, { categoria });
			onCategoriaChange(id, categoria);
		} finally {
			setGuardando(null);
		}
	};
							...
							<table className={styles.tabla}>
								...
								<tbody>
									{movs.map((m) => {
										...
										const esRegla =
											m.categoria_regla !== null && m.categoria_manual === null;
